...
implements ManyMindsConstants, AddRemoveAction {
    ...
    public
    RemoveAction(ManyMindsDocument t, String title) {
        super(title);
        trackingDoc = t;
    }
    ...
    actionPerformed(ActionEvent ae) {
        SwingUtilities.invokeLater(new Runnable() {
            public void
            run() {
                int i = JOptionPane.showConfirmDialog(null,"Are you sure you want to DELETE your "+getValue(NAME)+"?","Are you sure?", JOptionPane.YES_NO_OPTION);
                if (i == JOptionPane.YES_OPTION) {
                    if (XMLFactory.parseInt(trackingDoc.getValue()) > 0) {
                        trackingDoc.setValue(Integer.toString(XMLFactory.parseInt(trackingDoc.getValue()) - 1));
                    }
                }
            }
        });
