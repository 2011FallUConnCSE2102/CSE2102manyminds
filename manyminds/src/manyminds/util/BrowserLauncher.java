...
//    private static final String SECOND_WINDOWS_PARAMETER = "start";
    private static final String SECOND_WINDOWS_PARAMETER = "rundll32 url.dll,FileProtocolHandler";
    /**
   ...
   // private static final String THIRD_WINDOWS_PARAMETER = "\"\"";
    ...
    private static Object myWebKitComponent;
	...
	public static void openURL(String url) throws IOException {
		...
		if (myWebKitComponent != null) {
            if (myWebKitComponent.getClass().getName().endsWith("WebKitComponent")) {
                try {
                    Class[] params = {String.class};
                    String[] vals = {url};
                    Class wkcClass = Class.forName("manyminds.util.browser.WebKitComponent");
                    Method loadURLMethod = wkcClass.getDeclaredMethod("loadURL",params);
                    loadURLMethod.invoke(myWebKitComponent, vals);
                } catch (Throwable t) {
                    t.printStackTrace();
                }
            } else if (myWebKitComponent.getClass().getName().endsWith("CalHTMLPane")) {
                try {
		            Class[] params = {URL.class};
		            URL p = new URL(url);
		            URL[] vals = {p};
		            Class wkcClass = Class.forName("manyminds.util.browser.CalHTMLPane");
		            Method loadURLMethod = wkcClass.getDeclaredMethod("showHTMLDocument",params);
		            loadURLMethod.invoke(myWebKitComponent, vals);
		        } catch (Throwable t2) {
		            t2.printStackTrace();
		        }
